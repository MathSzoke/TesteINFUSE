...
public sealed class Credito
{
    ...
    public Credito(
        ...
        decimal baseCalculo)
    {
        if (string.IsNullOrWhiteSpace(numeroCredito)) throw new Errors.DomainException("numeroCredito é obrigatório");
        if (string.IsNullOrWhiteSpace(numeroNfse)) throw new Errors.DomainException("numeroNfse é obrigatório");
        if (string.IsNullOrWhiteSpace(tipoCredito)) throw new Errors.DomainException("tipoCredito é obrigatório");
